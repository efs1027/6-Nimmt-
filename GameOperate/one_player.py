...
class computer:#電腦類別
    ...
    def select_card(self):#選擇卡片
        ...
        self.card.pop()#拔掉
        self.draw_hand(False, False, False)
        pg.display.update()
        return self.selected_card#回傳
        
    def get_card(self, table, list_num):#收回卡片
        if len(self.point_card) == 1:
            card_list = table1.list_give_player(list_num, self.selected_card)#由函數取得該列並置換排頭 ## 第一個self刪除
    ...
    def count_bull(self):#算分數
        ...
        for num in self.point_card:
            self.bull += card_dict[num] # 這裡是 - 還是 + ?
        
    def draw_hand(self, gra, selected_card, display): # 手牌 ## gra:gradually 逐漸變化
        ...
        if len(self.card) > 0: #最後一回合前的出牌
            for card in self.card:
                ...
                if selected_card != False :
                    ...
                    if display:
                        num_font = text.render(str(selected_card), True, colors.BLACK) 
                ...
                if gra:
                    pg.display.update()
                    pg.time.delay(20)
            if selected_card != False and display != True: # 出牌階段延遲，亮牌階段不延遲
...
def play_easy():
    ...
    if start_stage:
        ...
        com1.draw_hand(True, False, False)
        com2.draw_hand(True, False, False)
        com3.draw_hand(True, False, False)
        player1.draw_hand(True, False)
    ...
    while game_keep_going :
        ...
        if select_stage:#選擇階段
            ...
            print(str(com1.card) +"  "+str(com1.selected_card))
            print(str(com2.card) +"  "+str(com2.selected_card))
            print(str(com3.card) +"  "+str(com3.selected_card))
            print(str(player1.card) +"  "+str(player1.selected_card))
            com1.draw_hand(False, card1, True)
            com2.draw_hand(False, card2, True)
            com3.draw_hand(False, card3, True)
            player1.draw_hand(False, card4)
            pg.display.update()
            pg.time.delay(20)  #200
            #變換階段變數
            select_stage = False
            place_stage = True
        ...
        if place_stage:#放置階段
            for card in sorted_cards_num:#由小而大看
                distanse = [0,0,0,0] # 初始由[0]改為[0,0,0,0]
                smaller = 0
                ...
                for distanses in distanse:#檢查每個距離 ## 這裡加s，叫distanse好像會有問題
                    #若小於零則計數增加一次
                    if distanses < 0:
                        smaller+=1
                #如果該牌不是小於全部列最後一張牌
